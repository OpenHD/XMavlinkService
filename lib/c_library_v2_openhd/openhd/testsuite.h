...
static void mavlink_test_openhd_system_telemetry(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_OPENHD_SYSTEM_TELEMETRY >= 256) {
            return;
        }
#endif
    ...
    mavlink_openhd_system_telemetry_t packet1, packet2;
        ...
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           ...
           memset(MAVLINK_MSG_ID_OPENHD_SYSTEM_TELEMETRY_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_OPENHD_SYSTEM_TELEMETRY_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_openhd_system_telemetry_encode(system_id, component_id, &msg, &packet1);
    ...
    MAVLINK_ASSERT(mavlink_get_message_info_by_name("OPENHD_SYSTEM_TELEMETRY") != NULL);
    MAVLINK_ASSERT(mavlink_get_message_info_by_id(MAVLINK_MSG_ID_OPENHD_SYSTEM_TELEMETRY) != NULL);
#endif
}
...
{
    mavlink_test_openhd_system_telemetry(system_id, component_id, last_msg);
